...
public class InventoryOverlayRenderHandler {
    ...
    private void renderTooltip(GuiRenderContext renderContext, @NotNull ItemStack itemStack, int x, int y) {
        ...
        List<net.minecraft.network.chat.Component> lines = itemStack.getTooltipLines(
        ...
        for (net.minecraft.network.chat.Component line : lines) {
            ...
            components.add(ClientTooltipComponent.create(text));
        }

        itemStack.getTooltipImage().ifPresent(image -> components.add(ClientTooltipComponent.create((TooltipComponent) image)));
