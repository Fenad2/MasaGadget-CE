...
public class InventoryOverlayRenderHandler {
    ...
    public void render(@NotNull GuiContext guiContext) {
        ...
        if (this.selectedSlot != InventoryOverlayRenderHandler.UN_SELECTED &&
                ...
                this.itemStack != null) {
            this.attachToSubShulkerBoxView(guiContext);
            this.attachToMainInventoryView(guiContext);
        }

        this.dropState();
    }

    private void attachToMainInventoryView(GuiContext guiContext) {
        if (!this.selectInventory) {
            this.renderSlotHighlight(guiContext, this.renderX, this.renderY);
            this.renderTooltip(guiContext, this.itemStack, this.renderX, this.renderY);
        }
    }

    private void attachToSubShulkerBoxView(GuiContext guiContext) {
        if (!this.selectInventory) {
            return;
        }
        ...
        if (this.subSelectedSlot != InventoryOverlayRenderHandler.UN_SELECTED &&
            ...
            this.renderSlotHighlight(guiContext, this.subRenderX, this.subRenderY);
            this.renderTooltip(guiContext, this.subItemStack, this.subRenderX, this.subRenderY);
    ...
    private void renderSlotHighlight(@NotNull GuiContext guiContext, int x, int y) {
        guiContext.fillGradient(
                x,
                y,
                x + 16,
                y + 16,
    ...
    private void renderTooltip(GuiContext guiContext, @NotNull ItemStack itemStack, int x, int y) {
        Minecraft mc = Minecraft.getInstance();
        guiContext.renderTooltip(
                mc.font,
                guiContext.itemTooltips(itemStack),
                x,
                y
        );
    }
